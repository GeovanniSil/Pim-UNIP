...
def cadastrar_aluno_e_notas_em_disciplina():
    """Solicita os dados do aluno (com validação de nome) e suas 3 notas."""
    ...
    while True:
        ...
        # Verifica se o nome não está vazio e contém apenas letras/espaços
    ...
    # Coleta os demais dados cadastrais (Inputs em Amarelo)
    ra = input(f"{COR_INPUT}RA do Aluno: {COR_RESET}").strip()
    turma = input(f"{COR_INPUT}Turma (Ex: ADS-A): {COR_RESET}").strip()
    ...
    try:
        ...
        with open(arquivo_csv, mode='r', newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                # Usa cor Verde se a média for maior ou igual a 6.0, e Amarelo caso contrário
                cor_media = Fore.GREEN if float(media_semestral) >= 6.0 else Fore.YELLOW
...
def main():
    ...
    while True:
        ...
        if acao == '1':
            # Cadastra o aluno e adiciona à fila da sua disciplina
            ...
            # Colore a média final calculada (Verde se >= 6.0, Amarelo se < 6.0)
